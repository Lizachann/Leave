            ...
            <table class="table table-bordered table-hover pt-3  ">
                ...
                @foreach($approvedLeaves as $approvedLeave)
                    <tr>
                        ...
                        <td>{{ \Carbon\Carbon::parse($approvedLeave->from_date)->format('d-m-Y') }}</td>
                        <td>{{ \Carbon\Carbon::parse($approvedLeave->to_date)->format('d-m-Y') }}</td>
                        <td>{{$approvedLeave->request_days}}</td>
                        <td>
                            @if($approvedLeave->hod_remark == 0)
                                Pending
                            @elseif($approvedLeave->hod_remark == 1)
                                Approved
                            @elseif($approvedLeave->hod_remark == 2)
                                Rejected
                            @endif
                        </td>
                        <td >
                            @if($approvedLeave->admin_remark == 0)
                                Pending
                            @elseif($approvedLeave->admin_remark == 1)
                                Approved
                            @elseif($approvedLeave->admin_remark == 2)
                                Rejected
                            @endif
                        </td>
                        ...
                        <td class="flex justify-center">
                            <button class=" px-3 py-1 rounded" >
                                <a href="{{ route('staff_view_leave_detail', ['id' => $approvedLeave->id]) }}"  >
