            ...
            <table class="table table-bordered table-hover pt-3  ">
                ...
                @foreach($rejectedLeaves as $rejectedLeave)
                    <tr>
                        ...
                        <td>{{ \Carbon\Carbon::parse($rejectedLeave->from_date)->format('d-m-Y') }}</td>
                        <td>{{ \Carbon\Carbon::parse($rejectedLeave->to_date)->format('d-m-Y') }}</td>
                        <td>{{$rejectedLeave->request_days}}</td>
                        <td>
                            @if($rejectedLeave->hod_remark == 0)
                                Pending
                            @elseif($rejectedLeave->hod_remark == 1)
                                Approved
                            @elseif($rejectedLeave->hod_remark == 2)
                                Rejected
                            @endif
                        </td>
                        <td >
                            @if($rejectedLeave->admin_remark == 0)
                                Pending
                            @elseif($rejectedLeave->admin_remark == 1)
                                Approved
                            @elseif($rejectedLeave->admin_remark == 2)
                                Rejected
                            @endif
                        </td>
                        ...
                        <td class="flex justify-center">
                            <button class=" px-3 py-1 rounded" >
                                <a href="{{ route('staff_view_leave_detail', ['id' => $rejectedLeave->id]) }}"  >
