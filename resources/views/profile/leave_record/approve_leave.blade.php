<h1 class="text-xl font-semibold text-[#4863A0] pb-4 ">Approved Record</h1>
<ol class="relative border-l border-gray-900 ">
    <li class="mb-10 ms-6">
        @foreach($approved_leaves as $approved_leave)
            <a class="absolute flex items-center justify-center w-10 h-10 bg-blue-300 rounded-full -start-5 ring-8 ring-white" href="{{ route($goto, ['id' => $approved_leave->id]) }}">
            ...
            <h3 class="flex items-center text-lg pt-1 font-semibold ml-5 ">
                {{ \Carbon\Carbon::parse($approved_leave->created_at)->format('d M Y') }}
            </h3>
            <h3 class="flex items-center text-md pt-1 font-semibold text-blue-900 ml-5 mb-2">
                {{$approved_leave->request_days}} Days
            </h3>
            ...
            <p class=" text-md font-normal text-gray-700 ml-5 mb-2">
                ...
                <label class="font-semibold">
                    {{ \Carbon\Carbon::parse($approved_leave->from_date)->format('d M Y') }}
                </label>
                @if($approved_leave->from_time == 'am')
                    <label class="font-semibold">AM</label>
                @elseif($approved_leave->from_time == 'pm')
                    <label class="font-semibold">AM</label>
                @endif
                To
                <label class="font-semibold">
                    {{ \Carbon\Carbon::parse($approved_leave->to_date)->format('d M Y') }}
                </label>
                @if($approved_leave->to_time == 'am')
                    <label class="font-semibold">AM</label>
                @elseif($approved_leave->to_time == 'pm')
                    <label class="font-semibold">PM</label>
                @endif
            </p>
            @if($approved_leave->admin_remark === 0 )
                <h3 class="block text-blue-800 text-sm font-semibold leading-none ml-5 mb-10">
                    Pending
                </h3>
            @elseif($approved_leave->admin_remark === 1 )
                <h3 class="block text-green-600 text-sm font-semibold leading-none ml-5 mb-10">
                    Approved
                </h3>
            @elseif($approved_leave->admin_remark === 2 )
